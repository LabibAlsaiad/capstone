...
class StepperMotorController(Node):
    def __init__(self):
        ...
        # self.slots = [2500, 5000, 7500]
        # self.slotsR= [7500, 5000, 2500]
        ...
        # self.pwm.start(2.5 + (10 / 18.0))  # Start PWM with 0% duty cycle
        ...
        # self.touch_publisher = self.create_publisher(Empty, '/touch', 10)

        # self.main_timer = self.create_timer(0.01, self.main_callback)
        # self.yaxis_motor_timer = self.create_timer(0.001, self.yaxis_motor_thread)
        # self.zaxis_motor_timer = self.create_timer(0.001, self.zaxis_motor_thread)
        ...
        # self.xaxis_proc = multiprocessing.Process(target=self.xaxis_motor_thread)
        # self.yaxis_proc = multiprocessing.Process(target=self.yaxis_motor_thread)
        # self.zaxis_proc = multiprocessing.Process(target=self.zaxis_motor_thread)
        ...
        # self.xaxis_proc.start()
        # self.yaxis_proc.start()
        # self.zaxis_proc.start()
    ...
    def leds_thread(self):
        ...
        while True:
            ...
            # self.get_logger().info(f"Max x value is : {self.xaxis_motor.position}")
            # self.get_logger().info(f"Max y value is : {self.yaxis_motor.position}")
    ...
    def main_thread(self):
        ...
        while (1):
            ...
            self.zaxis_motor.enable = 0
            # while (not (self.zaxis_motor.position > position + 200 )): 
            #     self.run_motor_to_position(self.zaxis_motor, 1000, position + 200)
            #     sleep(0.01)     
            ...
            for x in range(3):
                while not (self.slots[x] - self.tolerance <= self.xaxis_motor.position <= self.slots[x] + self.tolerance) or not (position - self.tolerance <= self.zaxis_motor.position <= position + self.tolerance):
                    ...
                    # if not (position - self.tolerance <= self.zaxis_motor.position <= position + self.tolerance):
            ...
            # while (not (self.zaxis_motor.position < 15000)):
            #     self.xaxis_motor.position = 0
            #     self.run_motor_to_position(self.zaxis_motor, 3000, 15000)
            #     sleep(0.01)

            # self.zaxis_motor.enable = 0

            # while 1:
            #     self.run_motor(self.yaxis_motor, 1, 1000)
            #     sleep(0.01)

            # if ((not self.xaxis_motor.initialized) or (not self.zaxis_motor.initialized)):
            #     self.initialize_motors()
            ...
            # self.run_motor_to_position(self.zaxis_motor, 3000, 3000)

            # self.run_motor(self.xaxis_motor, 1, 1000)
            # self.run_motor(self.zaxis_motor, 1, 3000)
    ...
    def run_motor_to_position(self, Motor, speed, position):
        ...
        # if position - self.tolerance < Motor.position:
        #     Motor.direction = 1  # Move backward
        #     Motor.speed = speed
        #     Motor.enable = 1
        # elif position + self.tolerance > Motor.position :
        #     Motor.direction = 0  # Move forward
        #     Motor.speed = speed
        #     Motor.enable = 1
        # else:
    ...
    def initialize_motors(self):
        ...
        if not self.xaxis_motor.initialized:
            ...
            # self.xaxis_motor.speed = 3000
            # self.xaxis_motor.direction = 1
            # self.xaxis_motor.enable = 1
        ...
        if not self.zaxis_motor.initialized:
            ...
            # self.zaxis_motor.speed = 3000
            # self.zaxis_motor.direction = 1
            # self.zaxis_motor.enable = 1
    ...
    def sensor_callback(self, msg):
        ...
        #     # f"Sensor Data - push buttin 1: {push_button1}, "
        #     f"Touch Sensor: {touch_sensor}"
        # )

        # Example motor control based on sensor data
        # if touch_sensor:
        #     self.run_motor(self.xaxis_motor, FORWARD, steps=10)
        #     self.xaxis_motor.position += 1
        #     self.get_logger().info(f"X Position: {self.xaxis_motor.position}")
